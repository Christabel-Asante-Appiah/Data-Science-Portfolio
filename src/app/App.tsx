...
import {
  ...
  ChevronDown, Plus, Tag, Film, Layers, Send, CheckCircle, Lock, Camera
} from "lucide-react";
...
const ADMIN_PASSWORD = "christabel2025"; // change this to whatever you want
  ...
  return (
    <div ref={ref} className="space-y-1.5">
      <div className="flex justify-between items-center">
        <span className="text-sm font-medium text-foreground" style={{ fontFamily: "DM Sans, sans-serif" }}>{name}</span>
        <span className="text-xs text-primary font-mono">{level}%</span>
      </div>
      <div className="h-1.5 bg-secondary rounded-full overflow-hidden">
        <motion.div
          className="h-full rounded-full"
          style={{ background: "linear-gradient(90deg, #f9b8cf, #d4608a)" }}
          initial={{ width: 0 }}
...
function AdminModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  return (
    <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      <motion.div
        className="relative bg-card border border-border rounded-2xl p-8 w-full max-w-sm shadow-xl"
      ...
      >
        ...
        <div className="flex flex-col items-center gap-3 mb-6">
          ...
          <p className="text-xs text-muted-foreground text-center">Enter your password to manage projects</p>
        </div>
        <input
          type="password"
          className="w-full bg-muted text-foreground rounded-xl px-4 py-3 text-sm border border-border focus:outline-none focus:border-primary transition-colors mb-2"
        ...
        <button
          onClick={attempt}
          className="w-full py-3 rounded-xl font-semibold text-sm text-white mt-1 hover:opacity-90 transition-opacity"
...
function UploadModal({ onClose, onAdd }: { onClose: () => void; onAdd: (p: Project) => void }) {
  ...
  const handleFile = (file: File) => {
    ...
    if (!isVideo && !isImage) { setError("Only image or video files are accepted."); return; }
  ...
  const handleSubmit = () => {
    ...
    if (!mediaUrl) { setError("Please upload an image or video."); return; }
    onAdd({ id: Date.now().toString(), title: title.trim(), description: description.trim(), tags, tools, mediaType, mediaUrl, date: new Date().toISOString().slice(0, 7) });
    onClose();
  };
    ...
    <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <h2 className="text-base font-semibold text-foreground" style={{ fontFamily: "Playfair Display, serif" }}>Add New Project</h2>
        ...
        <div className="p-6 space-y-4 max-h-[76vh] overflow-y-auto">
          ...
          >
            <input ref={fileRef} type="file" className="hidden" accept="image/*,video/*" onChange={e => { const f = e.target.files?.[0]; if (f) handleFile(f); }} />
            {mediaUrl
              ? mediaType === "video"
              ...
              : (
                <>
                  <div className="flex gap-4 text-primary/50"><ImageIcon size={28} /><Film size={28} /></div>
                  <p className="text-sm text-muted-foreground text-center">Drag and drop or click to upload<br /><span className="text-xs">Images and videos accepted</span></p>
                </>
              )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={addTag} className="bg-primary text-white px-3 rounded-xl text-sm hover:opacity-90 transition-opacity">+</button>
              </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={addTool} className="bg-secondary text-foreground px-3 rounded-xl text-sm hover:opacity-90 border border-border transition-opacity">+</button>
              </div>
              <div className="flex flex-wrap gap-1 mt-2">
                {tools.map(t => (
...
function Lightbox({ project, onClose }: { project: Project; onClose: () => void }) {
  return (
    <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-10" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      >
        <button onClick={onClose} className="absolute top-4 right-4 z-10 bg-white/80 text-foreground rounded-full p-1.5 hover:bg-white transition-colors shadow-sm"><X size={15} /></button>
        ...
        <div className="p-6 md:p-8">
          ...
          <h2 className="text-2xl font-bold text-foreground mb-3" style={{ fontFamily: "Playfair Display, serif" }}>{project.title}</h2>
...
function ProjectCard({ project, onClick }: { project: Project; onClick: () => void }) {
  return (
    <motion.div
      ...
      onClick={onClick}
      whileHover={{ y: -5, boxShadow: "0 12px 40px rgba(212,96,138,0.12)" }}
    ...
    >
      ...
      <div className="p-5">
        ...
        <h3 className="font-semibold text-foreground text-base mb-1.5 group-hover:text-primary transition-colors" style={{ fontFamily: "Playfair Display, serif" }}>{project.title}</h3>
...
function ContactForm() {
  ...
  const inputClass = "w-full bg-muted text-foreground rounded-xl px-4 py-3 text-sm border border-border focus:outline-none focus:border-primary transition-colors placeholder:text-muted-foreground";
    ...
    <div className="bg-card border border-border rounded-2xl p-8 shadow-sm">
      {sent ? (
        <motion.div className="flex flex-col items-center justify-center gap-3 py-10 text-center" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
          ...
          <p className="text-foreground font-semibold" style={{ fontFamily: "Playfair Display, serif" }}>Message sent!</p>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-mono text-muted-foreground uppercase tracking-wider block mb-1.5">Your Name</label>
              <input className={inputClass} value={form.name} onChange={set("name")} placeholder="Jane Smith" />
            </div>
            <div>
              <label className="text-xs font-mono text-muted-foreground uppercase tracking-wider block mb-1.5">Email Address</label>
          ...
          <div>
            ...
            <input className={inputClass} value={form.subject} onChange={set("subject")} placeholder="I would like to discuss a project..." />
          </div>
          <div>
            <label className="text-xs font-mono text-muted-foreground uppercase tracking-wider block mb-1.5">Message</label>
            <textarea className={`${inputClass} resize-none`} rows={5} value={form.message} onChange={set("message")} placeholder="Tell me about your project, role, or opportunity..." />
          </div>
          <button
            onClick={handleSend}
            disabled={!form.name || !form.email || !form.message}
            className="w-full py-3 rounded-xl font-semibold text-sm text-white flex items-center justify-center gap-2 transition-all hover:opacity-90 active:scale-[0.99] disabled:opacity-40 disabled:cursor-not-allowed"
            style={{ background: "linear-gradient(135deg, #f9b8cf, #d4608a)" }}
          >
...
function ProfilePicture({ isAdmin, src, onUpload }: { isAdmin: boolean; src: string | null; onUpload: (url: string) => void }) {
  ...
  return (
    <div className="relative inline-block mb-6">
      <div
        className={`w-28 h-28 rounded-full border-4 border-primary/30 overflow-hidden bg-secondary shadow-lg ${isAdmin ? "cursor-pointer" : ""}`}
      ...
      {isAdmin && (
        <div className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-primary flex items-center justify-center shadow-md border-2 border-background cursor-pointer" onClick={() => fileRef.current?.click()}>
    ...
    <div className="min-h-screen bg-background text-foreground" style={{ fontFamily: "DM Sans, sans-serif" }}>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${navScrolled ? "bg-background/90 backdrop-blur-md border-b border-border shadow-sm" : "bg-transparent"}`}>
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          {/* Admin upload button — only visible when logged in as admin */}
          {isAdmin && (
            <button
          ...
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div>
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                <ProfilePicture isAdmin={isAdmin} src={profilePic} onUpload={setProfilePic} />
              </motion.div>

              <motion.div
                className="inline-flex items-center gap-2 text-xs font-mono text-primary border border-primary/30 rounded-full px-3 py-1.5 mb-6 bg-primary/5"
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="flex gap-2 flex-wrap mb-10">
            {ALL_TAGS.map(tag => (
              <button
                ...
                className={`text-xs font-mono px-3 py-1.5 rounded-full border transition-all whitespace-nowrap ${
                  activeTag === tag
          ...
          {filtered.length === 0 ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map(p => (
                <ProjectCard key={p.id} project={p} onClick={() => setLightboxProject(p)} />
              ))}
            </div>
          )}
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <div className="space-y-4 text-muted-foreground text-sm leading-relaxed">
                <p>
                  I am a data scientist with 2 years of experience applying machine learning and advanced analytics to real business problems. My background spans three industries — banking, marketing, and telecommunications — giving me a sharp understanding of how data shapes customer behaviour, financial outcomes, and operational efficiency.
                </p>
                <p>
                  In banking, I worked on predictive models for credit risk and customer segmentation. In marketing, I built campaign attribution models and churn prediction systems that directly influenced retention strategy. In telecommunications, I applied data analytics to network performance and subscriber behaviour analysis.
                </p>
                <p>
                  I cover the full data stack: exploratory analysis, feature engineering, model development, visualisation, and communicating results to nontechnical stakeholders. I believe the best data scientist is also a great storyteller.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                >
                  ...
                  <h3 className="font-semibold text-sm text-foreground mb-1" style={{ fontFamily: "Playfair Display, serif" }}>{item.title}</h3>
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground text-sm max-w-md mx-auto leading-relaxed">
              Have a project, a dataset, or an open role? Fill in the form and I will get back to you.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-5 gap-8 items-start">
            ...
            <div className="md:col-span-2 space-y-4">
              <div className="bg-card border border-border rounded-2xl p-6">
                <p className="text-xs font-mono text-muted-foreground uppercase tracking-wider mb-4">Connect with me</p>
                <a
                  href={YOUR_LINKEDIN}
                  target="_blank"
                ...
                >
                  ...
                  <div>
                    <div className="text-sm font-medium text-foreground group-hover:text-primary transition-colors">LinkedIn</div>
                    <div className="text-xs text-muted-foreground">Christabel Asante Appiah</div>
                  </div>
                </a>
                <a
                  href={`mailto:${YOUR_EMAIL}`}
                  className="flex items-center gap-3 p-3 rounded-xl hover:bg-secondary transition-colors group mt-1"
                >
                  <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center">
                    <Mail size={16} className="text-primary" />
                  </div>
                  <div>
                    <div className="text-sm font-medium text-foreground group-hover:text-primary transition-colors">Email</div>
      ...
      <footer className="border-t border-border py-6">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-2">
          ...
          {/* Admin toggle — subtle, only you know to look here */}
          <button
            onClick={() => isAdmin ? setIsAdmin(false) : setShowAdminModal(true)}
            className="flex items-center gap-1 text-xs text-muted-foreground/40 hover:text-primary transition-colors font-mono select-none"
      ...
      <AnimatePresence>
        {showAdminModal && (
          <AdminModal onClose={() => setShowAdminModal(false)} onSuccess={() => setIsAdmin(true)} />
        )}
        {showUpload && isAdmin && (
          <UploadModal onClose={() => setShowUpload(false)} onAdd={p => setProjects(prev => [p, ...prev])} />
        )}
        {lightboxProject && (
